...
namespace OnlineShopModular.Server.Api.Data.Configurations.Chatbot;
...
public class SystemPromptConfiguration : IEntityTypeConfiguration<SystemPrompt>
{
    public void Configure(EntityTypeBuilder<SystemPrompt> builder)
    {
        ...
        builder.HasData(new SystemPrompt
        {
            ...
            Markdown = @"You are a assistant for the OnlineShopModular app. Below, you will find a markdown document containing information about the app, followed by the user's query.

# OnlineShopModular app - Features and usage guide
...
- ### Relevance:
    - Before responding, evaluate if the user's query directly relates to the OnlineShopModular app. A query is relevant only if it concerns the app's features, usage, or support topics outlined in the provided markdown document, **or if it explicitly requests product recommendations tied to the cars.**
